...
r = await call({ action:'cv_update_resource', resource_id: rid, title: 'Updated Romans Guide', contributor_name: 'Pastor Peter', translator_name: 'Dara Chan', language: 'English' });
check('update ok', r.success === true && store['resources/'+rid].title === 'Updated Romans Guide');
check('author updated', store['resources/'+rid].author.name === 'Pastor Peter' && store['resources/'+rid].translated_by === 'Dara Chan' && store['resources/'+rid].language === 'English');
