...
store['publicProfiles/uid-friend'] = { uid:'uid-friend', displayName:'Sok Dara', appUserId: 4242, church:'Grace Church', photoURL:'', createdAt:new Date('2026-08-29T08:00:00Z') };
store['publicProfiles/uid-newest'] = { uid:'uid-newest', displayName:'Newest Member', appUserId: 4243, church:'Hope Church', photoURL:'', createdAt:new Date('2026-08-31T08:00:00Z') };
...
r = await call({ action:'cv_get_recent_users' });
check('recent members are newest first', r.data.items[0].uid === 'uid-newest', r.data.items);
check('recent members include signup time', r.data.items[0].created_at === '2026-08-31T08:00:00.000Z', r.data.items[0]);
