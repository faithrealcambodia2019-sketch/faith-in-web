...
(() => {
  ...
  async function loadMembers() {
    ...
    try {
      ...
      list.innerHTML = (result.items || []).slice(0, 6).map(user => { const joined = Date.parse(user.created_at || ''); const isNew = Number.isFinite(joined) && Date.now() - joined < 7 * 24 * 60 * 60 * 1000; return `<li class="flex items-start gap-3 p-2 rounded-xl hover:bg-raised" data-user-uid="${esc(user.uid)}">${window.FILive.avatarMarkup(user)}<span class="min-w-0 flex-1"><a href="/profile?uid=${encodeURIComponent(user.uid)}" class="block text-[13.5px] font-semibold truncate">${esc(user.name)}${isNew ? ' <span class="ml-1 rounded-full bg-emerald-50 px-1.5 py-0.5 text-[9.5px] font-bold uppercase text-emerald-600">New</span>' : ''}</a><span class="block text-[12px] text-muted truncate">${esc(user.role || user.church || user.location || 'Faith In member')}</span></span><button class="btn btn-ghost !px-3 !py-1 !text-[12.5px] border border-line" data-live-message>Message</button></li>`; }).join('') || '<li class="text-[13px] text-muted p-2">No other members yet.</li>';
