          ...
          <button
            ...
            className={`px-3.5 py-1.5 rounded-full text-xs font-bold transition-all ${
              selectedCategory === cat
                ? "bg-[#2F5BEA] text-white shadow-xs"
            ...
            >
              ...
              >
                <span className="flex items-center gap-3">
                  <HelpCircle className={`w-4 h-4 shrink-0 ${isOpen ? "text-[#2F5BEA]" : "text-charcoal-400"}`} />
                  <span>{faq.question}</span>
                </span>
                <ChevronDown
                  className={`w-4 h-4 text-charcoal-500 transition-transform duration-200 shrink-0 ${
                    isOpen ? "rotate-180 text-[#2F5BEA]" : ""
                  }`}
                />
              </button>
              ...
              {isOpen && (
                <div className="px-5 pb-5 pt-1 space-y-2 border-t border-[#EAE7DC]/60 animate-fade-in">
                  {faq.khmerQuestion && (
                    <div className="text-xs font-khmer font-bold text-[#1E40AF] pt-2">
                      {faq.khmerQuestion}
                    </div>
                  )}
