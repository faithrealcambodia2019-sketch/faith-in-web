...
import { requireMember, verifyFirebaseToken } from "@/lib/verify-firebase-token";
    ...
    try {
      ...
      const jsonResponse = await handleUpload({
        ...
        onBeforeGenerateToken: async (pathname, clientPayload) => {
          let idToken = "";
          try {
            const payload = JSON.parse(clientPayload || "{}") as { idToken?: unknown };
          ...
          const member = await verifyFirebaseToken(idToken);
        ...
        onUploadCompleted: async () => {
          // Resource metadata is saved by the authenticated Firestore action
          // after the browser receives the completed Blob upload.
        },
      });
